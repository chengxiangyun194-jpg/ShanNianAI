Create Codex pet row `running-right` for `mimi`: exactly 8 full-body frames in one horizontal strip on flat pure magenta #FF00FF.

Use the attached canonical base for identity and the layout guide only for spacing. Same pet in every frame: 一只可爱的橘色虎斑猫桌宠，陪伴你编程. Preserve silhouette, face, palette, material, proportions, markings, and props.
